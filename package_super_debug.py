...
def create_package(output_filename="mastra_ai_super_debug.zip"):
    """Create a super debug package with all necessary files for the standalone application"""
    print("Creating Mastra AI Excel VBA Generator super debug package...")
    
    os.makedirs("dist", exist_ok=True)
    
    package_dir = "dist/super_debug_package"
    ...
    readme_file = f"{package_dir}/README_SUPER_DEBUG.txt"
    with open(readme_file, "w") as f:
        f.write("""Mastra AI Excel VBA Generator - スーパーデバッグバージョン

使用方法:
1. super_debug.bat をダブルクリックして実行
...
このスーパーデバッグバージョンは、詳細なエラーメッセージを表示し、ログファイルを生成します。
また、ディレクトリ構造やファイルの存在確認も行います。
問題が発生した場合は、これらの情報を開発者に提供してください。

問題が解決しない場合:
1. コマンドプロンプトを管理者として実行
2. 以下のコマンドを実行してPyQt6とPyQt6-WebEngineを再インストール:
   pip uninstall -y PyQt6 PyQt6-WebEngine
   pip install PyQt6 PyQt6-WebEngine
3. super_debug.bat を再度実行
""")
    ...
    print(f"Super debug package created at {output_path}")
    return output_path

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Package Mastra AI Excel VBA Generator Super Debug Version')
    parser.add_argument('--output', type=str, default="mastra_ai_super_debug.zip",
